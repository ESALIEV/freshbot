...
async def get_db() -> aiosqlite.Connection:
    ...
    if _db_connection is None:
        _db_connection = await aiosqlite.connect(DB)
...
async def init_db():
    ...
    """)
    for migration in [
        "ALTER TABLE products ADD COLUMN article TEXT DEFAULT ''",
...
# ──── ОБНОВЛЕНИЕ / УДАЛЕНИЕ (пункт 5: пересчёт уведомлений) ────
...
async def update_batch(batch_id: int, quantity: int, expiry_date: str):
    ...
    await db.execute(
        "DELETE FROM notifications WHERE batch_id = ? AND sent = 0", (batch_id,)
    )
    await db.commit()
...
async def create_invite_code(store_id: int, created_by: int, role: str = "worker", max_uses: int = 1) -> str:
    ...
    db = await get_db()
    await db.execute(
        "INSERT INTO invite_codes (code, store_id, role, created_by, expires_at, max_uses) VALUES (?, ?, ?, ?, ?, ?)",
        (code, store_id, role, created_by, expires_at.strftime("%Y-%m-%d %H:%M:%S"), max_uses)
    )
    await db.commit()
    return code
...
async def use_invite_code(code: str, user_id: int) -> dict | None:
    ...
    async with db.execute("""
        ...
        WHERE code = ? AND used_count < max_uses AND expires_at > datetime('now')
    """, (code,)) as cur:
